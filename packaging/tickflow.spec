...
# ── 容错收集包元数据: 包不存在时静默跳过 (供下方各 collect 段复用) ──
...
# ── 内置数据源插件 (akshare / tushare / fuyao): 动态 importlib 加载,
#    静态分析抓不到, 必须显式收集目录 + hiddenimports, 否则 frozen 后
#    插件注册失败 → 用户得自己 pip 装依赖。─────────────────────────────
for pname in ("akshare", "tushare", "fuyao"):
    pdir = ROOT / "backend" / "app" / "plugins" / pname
    if pdir.exists():
        datas += [(str(pdir), f"app/plugins/{pname}")]
hiddenimports += [
    "app.plugins.akshare.provider",
    "app.plugins.tushare.provider",
    "app.plugins.fuyao.provider",
]

# ── 插件第三方依赖 (akshare / tushare) 原生库 + 子模块 + 元数据 ──────
for pkg in ("akshare", "tushare"):
    d, b, h = collect_all(pkg)
    datas += d
    binaries += b
    hiddenimports += h
...
for pkg in (
    ...
    "python-dotenv", "fastexcel", "akshare", "tushare",
):
    datas += _safe_metadata(pkg)
